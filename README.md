...
基于 KingManiya/ComfyUI-LLM-text-processor 的增强版本，专注于 低显存 / 高内存 场景的本地 GGUF 模型推理优化，新增多项实用功能与前端交互改进。
...
重启 ComfyUI，在节点菜单中找到 LLM Text Processor。

🚀 快速上手（12GB 显存 + 64GB 内存）
将 performance_preset 设为 dense_27b_low_vram

确认 ffn_offload 自动变为 ffn_0_45

根据任务调整 ctx_size（建议 32768 起步）

连接图片输入：连上 image_1 后会自动出现 image_2

在 prompt 中输入提示词，运行即可
...
└── README.md
🙏 致谢
原项目：KingManiya/ComfyUI-LLM-text-processor
