...
# ===========================================================================
# 完整版节点：LLMTextProcessor
# ===========================================================================

class LLMTextProcessor:
    @classmethod
    def INPUT_TYPES(cls):
        image_optional = _build_image_inputs()
        ...
        return {
            "required": {
                ...
                "no_mmap": (
                    ...
                    {
                        "default": False,
                        "tooltip": "禁用内存映射（使用 --load-mode none）。",
                        "advanced": True,
                    },
                ),
                "cache_ram": (
    ...
    @classmethod
    def VALIDATE_INPUTS(cls, model, mmproj, system_prompt, mtp_draft, **kwargs):
        return True
    ...
    def generate(
        ...
        if mtp_draft not in current_mtps:
            raise ValueError(f"MTP draft not found: {mtp_draft}")

        model_path = full_model_path(model)
        mmproj_path = full_mmproj_path(mmproj)
        mtp_draft_path = full_mtp_path(mtp_draft)
...
# ===========================================================================
# 精简版节点：LLMTextProcessorLite
# 移除：system_prompt 文件选择、mtp_draft、mtp_mode、mtp_n_max
# ===========================================================================

class LLMTextProcessorLite:
    @classmethod
    def INPUT_TYPES(cls):
        image_optional = _build_image_inputs()
    ...
    FUNCTION = "generate"
    CATEGORY = "LLM Text Processor"
    TITLE = "LLM Text Processor (Lite)"

    @classmethod
    def VALIDATE_INPUTS(cls, model, mmproj, **kwargs):
        return True
    ...
    def generate(
        ...
        # Lite 版：不传 system_prompt_path、不传 mtp_draft_path、关闭 mtp
        command, cleanup_paths = build_command(
            model_path=model_path,
            mmproj_path=mmproj_path,
            mtp_draft_path=None,
...
NODE_CLASS_MAPPINGS = {
    ...
    "LLMTextProcessorLite": LLMTextProcessorLite,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "LLMTextProcessor": "LLM Text Processor",
    "LLMTextProcessorLite": "LLM Text Processor (Lite)",
}
